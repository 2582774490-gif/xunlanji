...
func _ready() -> void:
	...
	# cycles. Both launch templates also have native south-east cycles; the male
	# template has a native south-west cycle. Remaining directions retain their
	...
	if GameState.player.gender != "女":
		body.append_grid_clips(WALK_SOUTH_WEST_SHEET, 6, 1, {
