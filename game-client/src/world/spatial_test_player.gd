...
func _ready() -> void:
	...
	# cycles. Both launch templates also have native north-east cycles; the male
	# template has a native north-west cycle. Remaining directions retain their
	...
	if GameState.player.gender != "女":
		body.append_grid_clips(WALK_NORTH_WEST_SHEET, 6, 1, {
