...
func _check_yunlan_outskirts_scene() -> void:
	...
	_expect(outskirts.has_node("OldCaravanWaystation/Art") and (outskirts.get_node("OldCaravanWaystation/Art") as Sprite2D).texture != null and outskirts.old_caravan_waystation != null, "The old caravan road needs a real replaceable foreground waystation asset with a physical interaction point, instead of only procedural road marks.")
