...
func _check_costume_wardrobe_rules() -> void:
	...
	var north_west_walk_frames: Array = costume.get("walk_north_west_frames", [])
	_expect(north_west_walk_frames.size() == 6 and north_west_walk_frames.all(func(frame_path: Variant) -> bool: return ResourceLoader.exists(str(frame_path))), "Approved north-west walk must retain six project-local animation frames.")
	var north_walk_frames: Array = costume.get("walk_north_frames", [])
	_expect(north_walk_frames.size() == 6 and north_walk_frames.all(func(frame_path: Variant) -> bool: return ResourceLoader.exists(str(frame_path))), "Approved north walk must retain six project-local animation frames.")
	var north_east_walk_frames: Array = costume.get("walk_north_east_frames", [])
	_expect(north_east_walk_frames.size() == 6 and north_east_walk_frames.all(func(frame_path: Variant) -> bool: return ResourceLoader.exists(str(frame_path))), "Approved north-east walk must retain six project-local animation frames.")
	var east_walk_frames: Array = costume.get("walk_east_frames", [])
...
func _check_non_linear_story_weave() -> void:
	...
	_expect(GameState.choose_personal_story_origin("forge_watcher"), "Character creation should allow a player to explicitly choose a narrative opening independent of their root and physique.")
	_expect(str(GameState.personal_story_profile().get("id", "")) == "forge_watcher", "An explicitly selected opening must override only the automatic opening hint.")
	...
	_expect(not GameState.choose_personal_story_origin("storm_walker"), "A player must not be able to rewrite an opening route after real observations have created a personal history.")
...
func _check_codex_registry_ui() -> void:
	...
	_expect(market_text.contains("洛清") and market_text.contains("温行客") and market_text.contains("雾潮矿芯") and market_text.contains("潮息玉佩"), "Market UI must distinguish Luo Qing's market role from Wen Xingke's appraiser inventory.")
	market_ui.queue_free()
	GameState.player.sect_id = ""
	GameState.current_screen = GameState.Screen.SECT
	var sect_ui := preload("res://scenes/main.tscn").instantiate()
	add_child(sect_ui)
	await get_tree().process_frame
	await get_tree().process_frame
	var sect_text := _collect_label_text(sect_ui)
	_expect(sect_text.contains("百草谷") and sect_text.contains("白蘅") and sect_text.contains("百草调息篇"), "Sect UI must link Bai Heng to the existing Hundred Herbs Valley inner-sect transmission route.")
	sect_ui.queue_free()
	GameState.player = profile_before
...
func _check_yunlan_outskirts_scene() -> void:
	...
	_expect(outskirts.has_node("OldCaravanWaystation/Art") and (outskirts.get_node("OldCaravanWaystation/Art") as Sprite2D).texture != null and outskirts.old_caravan_waystation != null and outskirts.has_node("OldCaravanWaystation/Footprint/CollisionShape2D"), "The old caravan road needs a real replaceable foreground waystation asset with interaction and physical footprint, instead of only procedural road marks.")
	outskirts.player.position = outskirts.get_node("OldCaravanWaystation").position + Vector2(0, 140)
	var waystation_collision: Variant = outskirts.player.move_and_collide(Vector2(0, -240))
	_expect(waystation_collision != null, "The player must collide with the Old Caravan Waystation footprint instead of walking through its building art.")
	var mist_stream_terrain: Sprite2D = outskirts.get_node("MistStreamBanksChunk/TerrainArt")
	_expect(mist_stream_terrain.texture.resource_path.ends_with("yunlan_outskirts_mist_stream_banks_v01.png"), "Yunlan's mist-stream detail plane must use its dedicated Image 2 terrain painting rather than a repeated generic field texture.")
...
func _check_female_east_walk_cycle() -> void:
	...
	_expect(village.player.body.sprite_frames.get_frame_count("walk_south_east") == 6, "Female player template did not load its approved six-frame south-east walking cycle.")
	_expect(village.player.body.sprite_frames.get_frame_count("walk_south_west") == 6, "Female player template did not load its approved six-frame south-west walking cycle.")
	village.queue_free()
	await get_tree().process_frame
	GameState.player = profile_before
	GameState.profile_changed.emit()
...
func _check_mist_border_scene() -> void:
	...
	_expect(border.player.body.sprite_frames.get_frame_count("walk_south_east") == 6, "Male player template did not load its approved six-frame south-east walking cycle.")
	_expect(border.player.body.sprite_frames.get_frame_count("walk_south_west") == 6, "Male player template did not load its approved six-frame south-west walking cycle.")
	...
	_expect(border_minimap.world_bounds.size.x >= 11000.0 and border_minimap.landmarks.size() >= 12, "Mist Tide Border minimap does not represent the large region and its distributed fixed landmarks.")
	...
	_expect(ecology_profiles.any(func(profile: Dictionary): return str(profile.get("id", "")) == "mist_border_refuge_patrol" and str(profile.get("story_stance", "")) == "mender" and str(profile.get("story_branch", "")) == "mender_border_refuge_route"), "Mist Tide Border is missing the mender's old-checkpoint witness.")
	_expect(ecology_profiles.any(func(profile: Dictionary): return str(profile.get("id", "")) == "mist_border_ore_surveyor" and str(profile.get("story_stance", "")) == "seeker" and str(profile.get("story_branch", "")) == "seeker_border_ore_survey"), "Mist Tide Border is missing the seeker's ore-flat witness.")
	_expect(ecology_profiles.any(func(profile: Dictionary): return str(profile.get("id", "")) == "mist_border_wetland_scribe" and str(profile.get("story_stance", "")) == "witness" and str(profile.get("story_branch", "")) == "witness_border_wetland_log"), "Mist Tide Border is missing the witness's wetland record keeper.")
	_expect(RegionalSectorCatalog.sector_at("mist_border", Vector2(1140, 880)).get("id", "") == "old_checkpoint", "The mender witness must remain on the old checkpoint route.")
	_expect(RegionalSectorCatalog.sector_at("mist_border", Vector2(4520, 1300)).get("id", "") == "ore_flats", "The seeker witness must remain on the ore flats.")
	_expect(RegionalSectorCatalog.sector_at("mist_border", Vector2(7420, 920)).get("id", "") == "herb_wetland", "The witness record keeper must remain in the herb wetland.")
	...
	_expect(RegionalSectorCatalog.sector_at("mist_border", border.get_node("MistForestGate").position).get("id", "") == "mist_forest_road", "Mist Forest gate must stay on the forest approach rather than the old checkpoint.")
	_expect(RegionalSectorCatalog.sector_at("mist_border", border.get_node("MistBoneCreekGate").position).get("id", "") == "ore_flats", "Mist Bone Creek gate must sit on its water-cut ore-flat approach.")
	_expect(RegionalSectorCatalog.sector_at("mist_border", border.get_node("RedMapleRoadGate").position).get("id", "") == "cedar_mire", "Red Maple Road gate must begin at the cedar-mire road transition.")
	_expect(RegionalSectorCatalog.sector_at("mist_border", border.get_node("ThunderCliffGate").position).get("id", "") == "mist_highlands", "Thunder Listening Cliff gate must sit on the exposed highland route.")
	_expect(RegionalSectorCatalog.sector_at("mist_border", border.get_node("MistPortGate").position).get("id", "") == "outer_shoals", "Return Abyss Mist Port gate must remain on the outer-shoals sea route.")
	_expect(RegionalSectorCatalog.sector_at("mist_border", border.get_node("AbysswatchGate").position).get("id", "") == "tideward_hills", "Abysswatch Terrace gate must sit in the tideward hills.")
	_expect(RegionalSectorCatalog.sector_at("mist_border", border.get_node("AncientRidgeGate").position).get("id", "") == "tideward_hills", "Ancient Ridge gate must occupy the distant tideward-hills extension route.")
